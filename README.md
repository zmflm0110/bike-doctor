...
| 3 매일 아침 목록 서버 | 🟡 키만 오면 가동 | 목록 기록·다음 날 채점(SQLite), 7일 연속 예행연습, 생년·성별 없는 경우 대비, API 연결(주소만 넣으면 됨, 가짜 API 로 시험) | **공공데이터포털 인증키**·요청주소 확인 → 7일 실운영 |
| 4 웹앱 | 🟡 거의 | 5개 탭, 구조대 확인 → 정비 순위, QR 글자 이스케이프, 아이폰 PNG 아이콘, 화면·오프라인 자동 검사(CI), 집 와이파이 https | 아이폰 실기기 확인 |
| 5 현장 검증 | ⏳ 준비 완료 | 현장 조사 탭(사진 포함)·서버 API·검증 스크립트(95% 신뢰구간, 보고서용 표)·절차 문서 | 2주간 대여소 5곳+ 자전거 150대+ 조사, 친구 구조대 시범 |
| 6 넓히기 (전기차 충전기) | 🟡 뼈대 | 헛충전 연쇄 엔진·수집기·테스트, 공단 제안서 초안 | 인증키로 1~2주 수집 → 검증, 제안서 보낼지 결정 |
