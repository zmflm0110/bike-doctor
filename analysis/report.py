...
from engine.core import Rule, load_seoul, load_tashu, load_faults, mark, next_dud_table, prevented, lead_time
...
def main():
    ...
    lines = ["# 핵심 결과 (자동 생성: `python analysis/report.py`)", "",
             ...
             "대전은 생년·성별이 없어 같은 사람 재시도를 거르지 않는다(보수적이지 않음 — 서울 '거르기 전' 값과 비교).", "",
    ...
    for city, month, loader, path in SETS:
        ...
        lines.append(f"| {city} | {month} | {len(R):,} | {100*R['dud'].mean():.2f} | {cell(0)} | {cell(1)} | **{cell(2)}** | {cell(3)} | "
                     f"{P['per_day']} | {L.get('share_%', '—')} | {L.get('lead_h_median', '—')} | {L.get('victims_mean', '—')} |")
